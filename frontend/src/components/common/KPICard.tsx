...
export function KPICard({
  ...
  if (loading) {
    return (
      <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700 p-5 animate-pulse">
        ...
        <div className="h-7 bg-gray-200 dark:bg-gray-700 rounded w-20 mb-2" />
  ...
  const trendColor = trend === 'up' ? 'text-emerald-600' : trend === 'down' ? 'text-red-500' : 'text-gray-400 dark:text-gray-500'

  return (
    <div
      className={clsx(
        'bg-white dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700 p-5 transition-shadow',
    ...
    >
      ...
      <p className="text-2xl font-bold text-gray-900 dark:text-gray-100 tabular-nums mb-1">{value}</p>
      ...
      {change !== undefined && (
        <div className={clsx('flex items-center gap-1 mt-2', trendColor)}>
          ...
          <span className="text-xs font-medium">
            ...
            {changeLabel && <span className="text-gray-400 dark:text-gray-500 font-normal ml-1">{changeLabel}</span>}
          </span>
        </div>
      )}
    </div>
