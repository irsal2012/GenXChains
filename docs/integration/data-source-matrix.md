...
| Data domain | Primary source of truth | How GenXChains gets it | Initial migration? | Ongoing sync cadence | Notes |
|---|---|---|---|---|---|
...
| Demand plans (approved) | GenXChains (planning) | Created/approved in-app; optional publish-back | Optional (open plans only) | Continuous | Publish endpoint available after approval gates |
| Supply plans (approved) | GenXChains (planning) | Created/approved in-app; optional publish-back | Optional (open plans only) | Continuous | Publish endpoint available after approval gates |
| Forecast outputs | GenXChains ML engine | Generated in GenXChains | No | As-run | Not migrated from ERP; generated from historical data |
| Scenarios / what-if | GenXChains | Created in GenXChains | Optional (active only) | As-needed | Usually migrate only active scenarios |
| S&OP cycle workflow state | GenXChains | Created/managed in GenXChains | Optional | Ongoing | Migrate only if continuity is required |
| KPI definitions/targets | BI/Finance or GenXChains config | Manual seed/import | Yes | Monthly/quarterly review | Separate from KPI measured values |
| Audit logs | GenXChains (app events) | Auto-generated in app | No (usually) | Continuous | Legacy logs migrate only for compliance need |
